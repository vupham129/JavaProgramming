    ...
    public static void main(String[] args) {
        ...
        for (Map<String, Double> eachUnit : allUnits) {
            for (Map.Entry<String, Double> eachPair : eachUnit.entrySet()) {
                System.out.println("Java topic: "+eachPair.getKey()+" hours : "+eachPair.getValue());
            }
        }
        ...
        for (Map<String, Double> allUnit : allUnits) {
            for (Map.Entry<String, Double> eachPair : allUnit.entrySet()) {
                if(eachPair.getValue() >= 7){
                    System.out.println("Java topic that spend more than 7 hours: "+ eachPair.getKey());
                }
            }
        }
        System.out.println("------------------------------------------------");
        ...
        for (Map<String, Double> allUnit : allUnits) {
            for (Map.Entry<String, Double> eachPair : allUnit.entrySet()) {
                if(eachPair.getValue() < 3){
                    System.out.println("Java topics that spend less than 3 hours: "+eachPair.getKey());
                }
            }
        }
        System.out.println("------------------------------------------------");
        ...
        double maxHours = Integer.MIN_VALUE;
        String javaTopicMaxHours = "";
        for (Map<String, Double> allUnit : allUnits) {
            for (Map.Entry<String, Double> eachPair : allUnit.entrySet()) {
                if(eachPair.getValue() > maxHours){
                    maxHours = eachPair.getValue();
                    javaTopicMaxHours = eachPair.getKey();
                }
            }
        }
        System.out.println("javaTopicMaxHours = " + javaTopicMaxHours); //Custom Class
        System.out.println("------------------------------------------------");
        //    5. Which topic has the minimum hours?
        double minHours = Integer.MAX_VALUE;
        String javaTopicMinHours = "";
        for (Map<String, Double> allUnit : allUnits) {
            for (Map.Entry<String, Double> eachPair : allUnit.entrySet()) {
                if(eachPair.getValue() < minHours){
                    minHours = eachPair.getValue();
                    javaTopicMinHours = eachPair.getKey();
                }
            }
        }
        System.out.println("javaTopicMinHours = " + javaTopicMinHours); //Branching Statements
